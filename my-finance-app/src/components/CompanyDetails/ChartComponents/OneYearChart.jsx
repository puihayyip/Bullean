...
function OneYearChart() {
  ...
  const URL = `https://cloud.iexapis.com/stable/stock/${ticker}/chart/1y?token=${APIKEY}`;
  ...
  async function fetchData() {
    ...
    setState({
      ...state,
      chartData: { ...state.chartData, OneYearChartData: data },
    });
  }
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "1Y movement, 1 day interval",
      },
    },
    scales: {
      x: {
  ...
  const localChartData = state.chartData.OneYearChartData;
