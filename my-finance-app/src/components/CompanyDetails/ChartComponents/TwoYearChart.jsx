...
function TwoYearChart() {
  ...
  const URL = `https://cloud.iexapis.com/stable/stock/${ticker}/chart/2y?token=${APIKEY}&chartInterval=5`;
  ...
  async function fetchData() {
    ...
    setState({
      ...state,
      chartData: { ...state.chartData, TwoYearChartData: data },
    });
  }
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "2Y movement, 1 week interval",
      },
    },
    scales: {
      x: {
  ...
  const localChartData = state.chartData.TwoYearChartData;
