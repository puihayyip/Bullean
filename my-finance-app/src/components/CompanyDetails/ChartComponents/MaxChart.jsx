...
function MaxChart() {
  ...
  const URL = `https://cloud.iexapis.com/stable/stock/${ticker}/chart/max?token=${APIKEY}&chartInterval=5`;
  ...
  async function fetchData() {
    ...
    setState({
      ...state,
      chartData: { ...state.chartData, MaxChartData: data },
    });
  }
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Max movement, 1 week interval",
      },
    },
    scales: {
      x: {
  ...
  const localChartData = state.chartData.MaxChartData;
